...
class QwenR2RPanoramicDecoder(AutoregressiveDecoder):
    def __init__(self, policy: QwenR2RPanoramicPolicy):
        self.policy = policy
    ...
    def decode(
        ...
        *,
        ...
        if len(inference) == 4:
            ...
            raise RuntimeError("qwen_r2r_panoramic runner returned an invalid inference row")
        candidate_images = prefix.observation.metadata.get("candidate_images")
        candidate_count = (
            len(candidate_images) if candidate_images is not None else prefix.observation.images.shape[0] - 1
        )
...
class QwenR2RPanoramicPolicy(VLAPolicy):
    def __init__(self, name: str, runner: QwenR2RPanoramicRunner, profile: str):
        if profile != "panoramic":
            raise ValueError("QwenR2RPanoramicPolicy only accepts profile panoramic")
        super().__init__(
            VLAPolicyConfig(
                name=name,
                action_dim=2,
        ...
        self._decoder = QwenR2RPanoramicDecoder(self)
    ...
    def collate(self, observations, request_ids):
        ...
        return QwenR2RPanoramicBatch(list(observations), list(request_ids))
    ...
    def encode_prefix(self, batch, memory: MemoryState | None = None):
        ...
        if memory is None:
            ...
            memory = (
                QwenR2RPanoramicMemory(history_image_cache=cache)
                if cache is not None
                else QwenR2RPanoramicMemory()
            )
        return QwenR2RPanoramicPrefix(batch.observations[0], memory)


def _build_qwen_r2r_panoramic(
    name,
    checkpoint,
    max_new_tokens,
    overrides,
...
@register_policy("qwen2.5-vl-3b-r2r-panoramic")
def build_qwen_r2r_panoramic(
    checkpoint=None,
    *,
    max_new_tokens=1,
    attention_backend="torch_sdpa",
    ...
    **overrides,
    ...
    return _build_qwen_r2r_panoramic(
