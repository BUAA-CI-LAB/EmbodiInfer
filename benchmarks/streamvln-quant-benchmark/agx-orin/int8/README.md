# StreamVLN · agx-orin · INT8

仅 batch=1；[实验方法与环境](../../README.md)。本目录仅保留一份开启可用优化的 [config.json](config.json)。

INT8 native 后端；关闭语言 prefill 编译；开启视觉/decode CUDA Graph、历史特征缓存和 fast action decode。KV 缓存 32K，预热 33 帧。R2R/RxR 自动分进程依次运行，分别使用各自轨迹预热。

延迟 ms，吞吐 obs/s，内存为峰值 CUDA allocated GiB。

| 数据集 | 观测数 | E2E | Forward | 吞吐 | 内存 |
|---|---:|---:|---:|---:|---:|
| R2R | 2,997 | 2970.90 | 2933.40 | 0.337 | 14.78 |
| RxR | 3,879 | 3030.02 | 2992.01 | 0.330 | 15.47 |
