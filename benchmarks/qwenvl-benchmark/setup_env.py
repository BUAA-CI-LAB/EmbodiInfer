...
def main() -> None:
    ...
    prefixes = ("torch", "functorch", "triton", "nvidia", "cuda", "_cuda_")
    candidates = [path for path in source.iterdir() if path.name.startswith(prefixes)]
        ...
        json.dumps(
            {
                ...
                "note": f"{args.platform} runtime overrides LeRobot Torch upper bounds; runtime validation is required",
            },
            indent=2,
        )
        + "\n"
