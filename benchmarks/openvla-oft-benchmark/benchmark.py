"""openvla-oft offline benchmark: sampling, timing and inference in one file."""
...
def write_report(
    ...
    report = {
        ...
        "model_timing_contract": {
            "prefill_ms": "CUDA elapsed: DINOv2/SigLIP vision encoding, projection and Llama multimodal prompt prefill with KV collection",
            "decode_ms": "CUDA elapsed: one parallel Llama action-query forward against prefix KV and categorical selection; no autoregressive loop",
...
def build(
    ...
    """Measure the mainline categorical LIBERO OFT checkpoint, including detokenization."""
    checkpoint = Path(config["checkpoint"])
    statistics = json.loads((checkpoint / "dataset_statistics.json").read_text())
    if list(statistics) != ["libero_10_no_noops"]:
        raise ValueError("expected the LIBERO-10 SFT checkpoint and its sole normalization profile")
    policy = make_policy("openvla_oft", checkpoint=str(checkpoint), attention=config["attention"])
    # Cast on CPU before EngineCore moves the model: FP32 weights exceed 24 GiB.
    policy.to(dtype=getattr(torch, config["dtype"]))
    core = make_core(policy, config, device)
    compile_inference(policy, config, ("_llama_forward",))

    def infer(sample: LiberoSample, raw: dict[str, Any]) -> dict[str, Any]:
        batch = policy.collate([observation(sample, raw, cameras=1)], [sample.sample_id])
        batch = policy.pad(batch, 1).to(device, core.dtype)
        _, indices, timing = timed_model(
            lambda: policy.encode_prefix(batch),
            lambda prefix: policy.head.sample(
                policy.decode_action_logits(prefix, graphs=core._graphs, bucket=1), do_sample=False
            )[0],
            device,
        )
        actions = policy.head.tokens_to_actions(indices)[0].float().cpu()
        return {**output_record(actions, token_count=policy.n_tokens), "model_timing_ms": timing}
    ...
    return (
        infer,
        {
            ...
            "num_steps": 1,
            "camera_count": 1,
            "state_layout": "not consumed by the categorical checkpoint",
            "image_transform": "stored_opengl_vertical_flip_then_checkpoint_bicubic_224_DINOv2_SigLIP",
            "action_head": "RLinf categorical parallel tokens; not L1 regression OFT",
            "normalization_key": "libero_10_no_noops",
        },
