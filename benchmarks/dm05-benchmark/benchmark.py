"""dm05 offline benchmark: sampling, timing and inference in one file."""
...
def provenance(device: torch.device) -> dict[str, Any]:
    ...
    if "opendm" in sys.modules:
        source = Path(sys.modules["opendm"].__file__).resolve().parent
        source_digest = hashlib.sha256()
        for path in sorted(source.rglob("*.py")):
            source_digest.update(str(path.relative_to(source)).encode())
            source_digest.update(path.read_bytes())
        opendm_source = {"path": str(source), "python_sha256": source_digest.hexdigest()}
    return {
...
def build(
    ...
    """Load the official LIBERO experiment: two views, no state tokens, chunk ten."""
    policy = make_policy(
        "dm05",
        checkpoint=config["checkpoint"],
        norm_stats=str(Path(config["checkpoint"]) / "norm_stats.json"),
        robot_type="Franka",
        ...
        default_num_steps=config["num_steps"],
        output_action_dim=7,
        llm_attention=config["attention"],
        vision_attention=config["vision_attention"],
        action_attention=config["attention"],
        liger_kernel=config["liger_kernel"],
        prompt_style="opendm",
    )
    ...
    fused = {
        name: {
            ...
            "forward_module": getattr(module.forward, "__module__", ""),
        }
        for name, module in policy.named_modules()
        if type(module).__module__.startswith("liger_kernel")
        or getattr(module.forward, "__module__", "").startswith("liger_kernel")
    }
    if config["liger_kernel"] and not fused:
        raise RuntimeError("Liger was requested but no fused modules were installed")

    def infer(sample: LiberoSample, raw: dict[str, Any]) -> dict[str, Any]:
        obs = observation(sample, raw, cameras=2)
        obs.metadata["state_desc"] = ["joint"] * 6 + ["gripper"] * 2
        batch = policy.collate([obs], [sample.sample_id])
    ...
    def stats() -> dict[str, Any]:
        return {
            **runtime_details(core, policy),
            "liger_modules": fused,
            "llm_attention": policy._dm.model.vlm.config.text_config._attn_implementation,
            "vision_attention": policy._dm.model.vlm.config.vision_config._attn_implementation,
            "action_attention": policy._dm.model.action_expert.config._attn_implementation,
        }
    ...
    return (
        infer,
        {
            ...
            "camera_count": 2,
            "state_layout": "eef_pos3_axis_angle3_gripper2; normalized but add_state=false per official LIBERO experiment",
            "image_transform": "stored_opengl_vertical_flip_then_OpenDM_PixelTransform_and_checkpoint_processor",
        },
        stats,
    )
